...
import { classifyRunnerFailure } from "../vendor/helpers.js";
...
var WslBashExecutor = class extends LocalBashExecutor {
	static Config = LocalBashExecutor.Config.set("wslPath", z.string()).set("wslDistro", z.string()).set("sandbox", z.union([
		z.const("auto"),
		z.const("none"),
		z.const("bwrap")
	]).default("auto")).set("probeTimeoutMs", z.natural().default(3e4));
	...
	*/
	...
	async run(spec) {
		if (this.sandboxStance === "none" || !this.requireBwrapUsable()) return this.runArgv(spec, this.argv(spec));
		const policy = this.confinedPolicy(spec);
	...
	start(spec) {
		if (this.sandboxStance === "none" || !this.requireBwrapUsable()) return this.startArgv(spec, this.argv(spec));
		const policy = this.confinedPolicy(spec);
