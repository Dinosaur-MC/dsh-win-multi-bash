...
const DIALECT_FACTS = {
	posix: {
		...
		paths: "POSIX paths",
		env: "$VAR",
		toolchain: "the full Unix toolchain"
	},
	msys: {
...
const Config$1 = z.object({ enableRunInBackground: z.boolean().default(true) });
/**
...
*/
function shellDescription(dialect, backgroundEnabled, escalationModes) {
	...
	return `Execute a ${facts.shell} command (${facts.invoke}) and return its stdout/stderr. Each call runs in a fresh shell: no state (cwd, variables, functions) persists between calls — pass \`workdir\` instead of using \`cd\`. Paths use ${facts.paths}; read environment variables with ${facts.env}; ${facts.toolchain} is available. Non-zero exits are reported as \`[exit code: N]\`. Current harness environment facts are exposed through managed \`\$${DSH_ENV_PREFIX}*\` variables; inspect them when needed. Commands may run under a file sandbox; a blocked file operation is reported as \`[sandbox: file access denied under <mode> mode]\` — a policy denial, not a bug in the command; do not retry another way. Long output is truncated to its tail; the full output is saved to a file whose path is reported when available. ` + background + escalationTail(escalationModes);
}
/**
...
function escalationTail(escalationModes) {
	...
	return " Attempting a command the sandbox may deny is safe and expected: run it and read the marker rather than assuming the denial. When a command is denied and a wider mode would let it succeed, escalate immediately in the same turn — the one sanctioned exception to a denial: retry the exact same command once with `sandbox_permissions` (the narrowest wider mode that suffices) plus a one-sentence `justification`. Do not detour through chat to ask permission first — the approval prompt raised by that retry is how the user consents. If the session states approval prompts are disabled, there is no exception: a denial is final — do not set `sandbox_permissions`. Never escalate speculatively: ground the request in a real denial — normally the one this command just hit; escalating up front is fine only when this session already denied the same access. A rejected escalation is final for that command — stop and explain, never work around it — but it does not forbid attempting or escalating other commands later.";
}
...
function defineShellTool(def) {
	return {
		...
		apply(ctx, config = {}) {
			...
			if (defaultMode !== void 0 && sandboxPolicy === void 0) throw new Error(`tool-${def.toolName}: the mounted bash executor confines but ctx.sandboxPolicy is missing`);
			...
			*/
			...
			ctx.tools.register(defineTool({
				...
				async execute(args, exec) {
					...
					const approvedMode = args.sandbox_permissions !== void 0 && args.justification !== void 0 ? await approveShellEscalation(args.sandbox_permissions, args.justification, exec, standingPolicy) : void 0;
					...
					const request = {
						...
						...def.shell !== void 0 ? { shell: def.shell } : {},
						dshEnv,
...
const bashTool = defineShellTool({
	toolName: "bash",
	dialect: "posix"
});
const name = bashTool.name;
const inject = bashTool.inject;
const Config = bashTool.Config;
const apply = bashTool.apply;
//#endregion
export { Config, apply, defineShellTool, inject, name };
